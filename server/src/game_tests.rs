...
mod tests {
    ...
    fn get_player_world_pos(game: &Game, player_id: Uuid) -> Option<WorldPos> {
        game.players.get(&player_id).map(|player| match player.location {
    ...
    fn simulate_walk_to_tile(game: &mut Game, player_id: Uuid, target_tile: TilePos) -> Vec<TileEvent> {
    ...
    fn process_tile_events_sync(game: &mut Game, events: Vec<TileEvent>) -> Vec<shared::ServerMessage> {
    ...
    fn assert_player_outside_world(game: &Game, player_id: Uuid) {
        ...
        match &player.location {
            ...
            PlayerLocation::InsideMech { mech_id, .. } => {
                panic!("Player {} is in mech {}, expected to be outside", 
                    player_id, mech_id);
            }
        }
    }
    ...
    fn test_game_initialization() {
        ...
        let red_mechs: Vec<_> = game.mechs.values().filter(|m| m.team == TeamId::Red).collect();
        let blue_mechs: Vec<_> = game.mechs.values().filter(|m| m.team == TeamId::Blue).collect();
    ...
    fn test_player_can_spawn() {
        ...
        assert!(game.players.contains_key(&player_id), "Player should be in game");
    ...
    fn test_player_can_walk_to_mech_door() {
        ...
        // Get red team mech
        let red_mech = get_team_mech(&game, TeamId::Red)
        ...
        assert!((player_pos.x - expected_pos.x).abs() < 1.0, 
            "Player X position should be close to door");
        assert!((player_pos.y - expected_pos.y).abs() < 1.0, 
    ...
    fn test_player_enters_mech_on_door_tile() {
        ...
        if let Some(player) = game.players.get(&player_id) {
            ...
            match &player.location {
                PlayerLocation::InsideMech { mech_id: entered_mech_id, floor, pos } => {
                    println!("SUCCESS: Player entered mech {} on floor {} at {:?}", 
                        entered_mech_id, floor, pos);
                    assert_eq!(*entered_mech_id, mech_id, "Player should be in the red mech");
                }
                PlayerLocation::OutsideWorld(pos) => {
    ...
    fn test_player_denied_entry_to_enemy_mech() {
        ...
        let blue_mech = get_team_mech(&game, TeamId::Blue)
    ...
    */
    ...
    fn test_door_positions_are_on_transition_tiles() {
        ...
        for mech in game.mechs.values() {
            ...
            println!("Mech at {:?} has doors at {:?} and {:?}", 
            ...
            if let Some(shared::tile_entity::TileContent::Static(static_tile)) = left_tile {
                match static_tile {
                    shared::tile_entity::StaticTile::TransitionZone { transition_type, .. } => {
