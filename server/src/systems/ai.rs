use shared::*;
use crate::{Game, systems::GameSystem};
use ai::{AIManager, AICommand, GameView};
use uuid::Uuid;
...
impl AISystem {
    ...
    pub fn add_ai_player(&mut self, difficulty: f32, personality: Option<ai::Personality>, red_count: usize, blue_count: usize) -> (Uuid, crate::game::Player) {
        ...
        let team = if red_count <= blue_count { TeamId::Red } else { TeamId::Blue };
        ...
        // Track AI info
        self.ai_players.insert(ai_id, AIPlayerInfo {
            name,
            team,
            mech_id: None,
        });
    ...
    fn create_game_view(&self, game: &Game, ai_id: Uuid) -> GameView {
        ...
        let players: Vec<ai::PlayerView> = game.players.iter()
            .map(|(id, p)| {
        ...
        let mechs: Vec<ai::MechView> = game.mechs.iter()
            .map(|(id, m)| {
                // Convert stations to StationView
                let stations: Vec<ai::StationView> = m.stations.values()
        ...
        let projectiles: Vec<ai::ProjectileView> = game.projectiles.iter()
            .map(|(id, p)| {
                // Find the team of the owner mech
                let owner_team = game.mechs.get(&p.owner_mech_id)
                    .map(|m| m.team)
        ...
        // Get team info
        let team_members: Vec<Uuid> = game.players.values()
        ...
        let team_info = ai::TeamInfo {
            ...
            mech_count: game.mechs.values().filter(|m| m.team == player.team).count(),
            total_resources: game.mechs.values()
    ...
    fn process_ai_commands(&self, commands: Vec<AICommand>) -> Vec<ServerMessage> {
        ...
        for command in commands {
            match command {
                ...
                AICommand::PressButton { player_id, button_index } => {
                ...
                AICommand::EngineControl { player_id, movement } => {
                        ...
                        AICommand::Move { player_id, movement } => {
                            if let Some(player) = game.players.get_mut(&player_id) {
                                ...
                                if let PlayerLocation::OutsideWorld(mut position) = player.location {
                                    ...
                                    // Keep in bounds
                                    position.x = position.x.max(0.0).min((ARENA_WIDTH_TILES as f32) * TILE_SIZE);
                                    position.y = position.y.max(0.0).min((ARENA_HEIGHT_TILES as f32) * TILE_SIZE);
                        ...
                        AICommand::PressButton { player_id, button_index } => {
                            if let Some(player) = game.players.get(&player_id) {
                                if let Some(station_id) = player.operating_station {
                                    ...
                                    let mech_station_info = game.mechs.iter_mut()
                                        .find_map(|(mech_id, mech)| {
                                            mech.stations.get(&station_id)
                                        ...
                                        match station_type {
                                            StationType::WeaponLaser => {
                                                if let Some(_mech) = game.mechs.get(&mech_id) {
                                                    // Fire laser weapon
                                                    // For now, log the action
                                                    log::debug!("AI {} pressed button {} on laser station", player_id, button_index);
                                                }
                                            }
                                            ...
                                            StationType::Shield => {
                                                // Activate shield
                                                // For now, log the action
                                                log::debug!("AI {} pressed button {} on shield station", player_id, button_index);
                                            }
                                            _ => {}
                                        }
                                    }
