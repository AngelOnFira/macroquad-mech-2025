...
impl CombatSystem {
    ...
    fn handle_mech_destroyed(&self, game: &mut Game, mech_id: Uuid, messages: &mut Vec<ServerMessage>) {
        // Eject all players from destroyed mech
        let mut players_to_eject = Vec::new();
        
        for (player_id, player) in game.players.iter() {
            if let PlayerLocation::InsideMech { mech_id: player_mech_id, .. } = player.location {
    ...
    fn process_weapon_targeting(&self, game: &Game, firing_mech_id: Uuid, weapon_range: f32) -> Option<Uuid> {
    ...
    fn create_combat_effects(&self, game: &mut Game, weapon_type: StationType, from_pos: WorldPos, to_pos: Option<WorldPos>) {
        match weapon_type {
