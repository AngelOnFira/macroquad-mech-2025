...
impl TileBehaviorSystem {
    ...
    fn process_auto_interactions(
        ...
        for (entity_id, auto) in &entities.auto_interacts {
            ...
            for (player_id, player) in players {
                // Check conditions
                let conditions_met = auto.conditions.iter().all(|cond| {
                    match cond {
                        InteractionCondition::PlayerNotCarrying => {
                            player.carrying_resource.is_none()
                        }
...
impl GameSystem for TileBehaviorSystem {
    fn update(&mut self, game: &mut Game, delta_time: f32) -> Vec<ServerMessage> {
        ...
        for event in events {
            match event {
                TileEvent::ResourcePickedUp { resource_entity, actor, resource_type } => {
                    // Update game state
                ...
                TileEvent::ProximityTriggered { entity, actor, .. } => {
                    ...
                    // For now, just log it
                    log::debug!("Proximity trigger: entity {:?} triggered by {:?}", entity, actor);
                }
                TileEvent::AutoInteractionTriggered { entity, actor, action } => {
                    log::debug!("Auto interaction: {:?} on entity {:?} by {:?}", action, entity, actor);
        ...
        for (_, trigger) in &mut game.entity_storage.proximity_triggers {
            ...
            trigger.last_triggered.retain(|_, time| {
                self.time_elapsed - *time < trigger.cooldown * 2.0
            });
        }
...
mod tests {
    ...
    fn test_proximity_trigger() {
        ...
        entities.positions.insert(entity_id, Position {
            tile: TilePos::new(5, 5),
            world: WorldPos::new(80.0, 80.0),
            floor: None,
            mech_id: None,
        });
        entities.proximity_triggers.insert(entity_id, ProximityTrigger {
            range: 32.0,
