...
impl HatManager {
    ...
    pub fn update_hat(&mut self, perception: &Perception) {
        ...
        if self.reactive_hat.is_none() {
            ...
            if let Some((best_hat, score)) = self
                .hat_scores
                .iter()
    ...
    fn check_reactive_conditions(&self, perception: &Perception) -> Option<Hat> {
        ...
        if perception.team_state.resource_status.scarcity_level > 0.99 {
            log::debug!(
                "ResourceRush triggered: scarcity_level = {}",
                perception.team_state.resource_status.scarcity_level
            );
        ...
        if matches!(
            perception.my_state.health_status,
            HealthStatus::Critical | HealthStatus::Damaged
    ...
    fn evaluate_hat_scores(&mut self, perception: &Perception) {
        ...
        let engineer_score = if perception
            .team_state
            .mech_health
            .values()
        ...
        let scout_score =
            if perception.threats.is_empty() && perception.environment.nearby_resources.len() < 3 {
                0.5
            } else {
                0.2
            };
        ...
        self.available_tasks.insert(
            ...
            vec![
                Task {
                    ...
                    priority: 0.7,
                    action: TaskAction::CollectResource {
                        resource_type: None,
                    },
                    requirements: TaskRequirements {
                        location: Some(LocationRequirement::Outside),
