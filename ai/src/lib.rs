...
impl AIManager {
    ...
    pub fn initialize_ais(&mut self) -> Vec<(String, TeamId)> {
        ...
        for i in 0..self.config.ai_count {
            ...
            let controller: Box<dyn AIController> = if self.config.difficulty > 0.7 {
                Box::new(utility::UtilityAI::new(ai_id, personality, self.config.difficulty))
            } else {
                Box::new(utility::SimpleAI::new(ai_id, personality, self.config.difficulty))
            };
    ...
    pub fn update(&mut self, game_view: &GameView, delta_time: f32) -> Vec<AICommand> {
        ...
        for (ai_id, controller) in &mut self.controllers {
            if let Some(perception) = perceptions.get(ai_id) {
                ...
                let relevant_messages: Vec<_> = messages.iter()
    ...
    pub fn get_debug_info(&self, ai_id: Uuid) -> Option<AIDebugInfo> {
        self.controllers.get(&ai_id).map(|controller| controller.get_debug_info())
    }
...
#[derive(Debug, Clone)]
pub enum AICommand {
    Move { player_id: Uuid, movement: (f32, f32) },
    PressButton { player_id: Uuid, button_index: u8 },
    ExitMech { player_id: Uuid },
    EngineControl { player_id: Uuid, movement: (f32, f32) },
}