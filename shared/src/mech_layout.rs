...
use crate::tile_entity::{StaticTile, TileContent};
use crate::{
    uuid_gen::new_uuid, StationType, TilePos, FLOOR_HEIGHT_TILES, FLOOR_WIDTH_TILES, MECH_FLOORS,
};
...
impl MechLayoutGenerator {
    ...
    pub fn create_mech_interior(stations: &mut HashMap<Uuid, MechStation>) -> MechInterior {
        ...
        for floor_idx in 0..MECH_FLOORS {
            let mut tiles = vec![
                vec![TileContent::Empty; FLOOR_WIDTH_TILES as usize];
                FLOOR_HEIGHT_TILES as usize
            ];
    ...
    fn get_stations_for_floor(floor_idx: usize) -> Vec<(TilePos, StationType)> {
        match floor_idx {
            ...
            1 => vec![
                (
                    TilePos::new(STATION_POSITIONS[1][0].0, STATION_POSITIONS[1][0].1),
                    StationType::WeaponLaser,
                ),
                (
                    TilePos::new(STATION_POSITIONS[1][1].0, STATION_POSITIONS[1][1].1),
...
mod tests {
    ...
    fn test_custom_floor_creation() {
        ...
        assert!(matches!(
            floor.get_tile(TilePos::new(2, 2)),
            Some(TileContent::Empty)
        ));
        assert!(matches!(
            floor.get_tile(TilePos::new(4, 4)),
            Some(TileContent::Empty)
        ));
    }
}
