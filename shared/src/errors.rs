use thiserror::Error;
use uuid::Uuid;
use crate::types::{TeamId, ResourceType, StationType};
...
pub enum GameError {
    ...
    WrongTeamMech { player_team: TeamId, mech_team: TeamId },
    
    #[error("Mech {id} has insufficient health: {current}/{required}")]
    InsufficientMechHealth { id: Uuid, current: u32, required: u32 },
