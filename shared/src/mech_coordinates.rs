use crate::{TilePos, FLOOR_WIDTH_TILES, FLOOR_HEIGHT_TILES, MECH_FLOORS, MECH_SIZE_TILES};
use crate::coordinates::{WorldPos, MechDoorPositions};
...
impl MechInteriorCoordinates {
    ...
    pub fn crosses_window(outside_pos: WorldPos, mech_pos: TilePos, _interior_floor: u8, _interior_pos: TilePos) -> Option<Direction> {
    ...
    pub fn distance_to_interior(outside_pos: WorldPos, mech_pos: TilePos, floor: u8, interior_pos: TilePos) -> f32 {
...
impl MechVisionUtils {
    ...
    pub fn can_see_into_mech(
        ...
        let distance = MechInteriorCoordinates::distance_to_interior(
            viewer_pos, mech_pos, target_floor, target_interior_pos
        );
        
        // Too far away
        if distance > 200.0 { // Roughly 6-7 tiles
            return (false, 0.0);
        }
    ...
    pub fn get_potentially_visible_interior_tiles(
        ...
        for floor in 0..MECH_FLOORS as u8 {
            for y in 0..FLOOR_HEIGHT_TILES {
                for x in 0..FLOOR_WIDTH_TILES {
                    let interior_pos = TilePos::new(x, y);
                    let (can_see, visibility) = Self::can_see_into_mech(
                        viewer_pos, mech_pos, floor, interior_pos
                    );
...
mod tests {
    ...
    fn test_interior_to_world_conversion() {
        ...
        let world_pos_floor_0 = MechInteriorCoordinates::interior_to_world(mech_pos, 0, interior_pos);
        assert_eq!(world_pos_floor_0, TilePos::new(12, 13));
        
        // Floor 1 should be offset vertically
        let world_pos_floor_1 = MechInteriorCoordinates::interior_to_world(mech_pos, 1, interior_pos);
        assert_eq!(world_pos_floor_1, TilePos::new(12, 13 + FLOOR_HEIGHT_TILES + 1));
    }
    ...
    fn test_world_to_interior_conversion() {
        ...
        let interior_floor_1 = MechInteriorCoordinates::world_to_interior(world_pos_floor_1, mech_pos);
    ...
    fn test_mech_bounds_checking() {
        ...
        assert!(MechInteriorCoordinates::is_within_mech_bounds(inside_pos, mech_pos));
        
        // Outside bounds
        let outside_pos = TilePos::new(5, 5);
        assert!(!MechInteriorCoordinates::is_within_mech_bounds(outside_pos, mech_pos));
    }
    ...
    fn test_door_detection() {
        ...
        assert!(MechInteriorCoordinates::is_mech_door_from_outside(doors.left_door, mech_pos));
        assert!(MechInteriorCoordinates::is_mech_door_from_outside(doors.right_door, mech_pos));
        
        // Should not detect non-door tiles
        assert!(!MechInteriorCoordinates::is_mech_door_from_outside(TilePos::new(0, 0), mech_pos));
    }
    ...
    fn test_vision_into_mech() {
        ...
        let (can_see, visibility) = MechVisionUtils::can_see_into_mech(
            viewer_pos, mech_pos, 0, interior_pos
        );
        ...
        let (can_see_far, _) = MechVisionUtils::can_see_into_mech(
            far_viewer_pos, mech_pos, 0, interior_pos
        );
