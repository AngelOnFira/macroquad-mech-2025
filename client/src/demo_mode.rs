...
impl DemoMode {
    ...
    fn create_mech_interior(&mut self) {
        ...
        for y in 0..10 {
            for x in 0..10 {
                let is_wall = x == 0 || x == 9 || y == 0 || y == 9;
                let is_window =
                    is_wall && ((x == 5 && y == 0) || (x == 0 && y == 5) || (x == 9 && y == 5));
                ...
                if self.visible_tiles.contains(&(*x, *y)) {
                    ...
                    for i in 1..=8 {
                        ...
                        if i > 2 {
                            ...
                            self.visible_tiles
                                .insert((x + dx * i + perp_dx, y + dy * i + perp_dy));
                            self.visible_tiles
                                .insert((x + dx * i - perp_dx, y + dy * i - perp_dy));
                        }
                    }
    ...
    pub async fn run(&mut self) {
        loop {
            ...
            if let Some((_, _, tile)) = self
                .tiles
                .iter()
                .find(|(x, y, _)| *x == player_tile.0 && *y == player_tile.1)
            {
                let tile_desc = match tile {
                    TileVisual::Floor { material, wear } => {
                        format!("Floor ({:?}, wear: {})", material, wear)
                    }
                    TileVisual::Wall { material } => format!("Wall ({:?})", material),
                    TileVisual::Window { facing, broken } => {
                        format!("Window ({:?}, broken: {})", facing, broken)
                    }
                    TileVisual::Station {
                        station_type,
                        active,
                    } => format!("Station ({:?}, active: {})", station_type, active),
                    TileVisual::Turret { facing, firing } => {
                        format!("Turret ({:?}, firing: {})", facing, firing)
                    }
                    TileVisual::TransitionFade { progress } => {
                        format!("Transition ({}%)", (progress * 100.0) as i32)
                    }
                };
                draw_text(
                    &format!("Current tile: {}", tile_desc),
                    10.0,
                    screen_height() - 20.0,
                    18.0,
                    WHITE,
