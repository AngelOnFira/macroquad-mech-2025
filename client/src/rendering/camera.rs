...
use shared::{
    coordinates::{WorldPos, TilePos, ScreenPos, TileRegion},
    constants::{TILE_SIZE, ARENA_WIDTH_TILES, ARENA_HEIGHT_TILES},
};
...
impl ScreenSpace {
    ...
    pub fn is_mouse_over_area(
        ...
        mouse_world.x >= world_pos.x && mouse_world.x <= world_pos.x + width &&
        mouse_world.y >= world_pos.y && mouse_world.y <= world_pos.y + height
    }
