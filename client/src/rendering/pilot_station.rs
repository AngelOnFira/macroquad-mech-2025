...
fn render_area_view(
    ...
    if let Some(mech) = operating_mech {
        ...
        // Draw grid
        draw_grid(
            map_x, map_y, map_width, map_height, world_left, world_top, MAP_ZOOM,
        );
        ...
        for (mech_id, other_mech) in &game_state.mechs {
            ...
            if screen_x < map_x
                || screen_x > map_x + map_width
                || screen_y < map_y
                || screen_y > map_y + map_height
            {
                continue;
            }
...
pub fn is_pilot_window_clicked(
    game_state: &GameState,
    mouse_x: f32,
    mouse_y: f32,
) -> PilotWindowClick {
    ...
    if mouse_x >= window_x
        && mouse_x <= window_x + PILOT_WINDOW_WIDTH
        && mouse_y >= window_y
        && mouse_y <= window_y + PILOT_WINDOW_HEIGHT
    {
        // Check close button
        let close_x = window_x + PILOT_WINDOW_WIDTH - 30.0;
        if mouse_x >= close_x
