use shared::{MechInteriorCoordinates, WorldPos, TilePos, TILE_SIZE, FLOOR_WIDTH_TILES, FLOOR_HEIGHT_TILES, MECH_FLOORS};
...
impl SpatialDebugRenderer {
    ...
    pub fn render_coordinate_mapping(&self, 
        ...
        if let Some(mech) = game_state.mechs.get(&mech_id) {
            ...
            draw_line(
                interior_screen_x, interior_screen_y,
                world_screen_x, world_screen_y,
                3.0,
                YELLOW
            );
    ...
    pub fn render_mech_spatial_bounds(&self, 
                                    game_state: &GameState,
                                    cam_x: f32,
                                    cam_y: f32) {
    ...
    fn render_single_mech_bounds(&self, mech_pos: TilePos, cam_x: f32, cam_y: f32) {
        ...
        for floor in 0..MECH_FLOORS as u8 {
            ...
            let floor_min = MechInteriorCoordinates::interior_to_world(
                mech_pos, floor, TilePos::new(0, 0)
            );
            let floor_max = MechInteriorCoordinates::interior_to_world(
                mech_pos, floor, TilePos::new(FLOOR_WIDTH_TILES - 1, FLOOR_HEIGHT_TILES - 1)
            );
    ...
    pub fn render_door_entry_points(&self, 
                                  game_state: &GameState,
                                  cam_x: f32,
                                  cam_y: f32) {
        ...
        for mech in game_state.mechs.values() {
            ...
            for door_tile in doors.door_tiles() {
                ...
                let mech_center_x = mech_center.x + (shared::MECH_SIZE_TILES as f32 * TILE_SIZE) / 2.0;
    ...
    pub fn render_floor_offsets(&self, 
                               game_state: &GameState,
                               cam_x: f32,
                               cam_y: f32) {
        ...
        for mech in game_state.mechs.values() {
            for floor in 0..MECH_FLOORS as u8 {
                let base_pos = MechInteriorCoordinates::interior_to_world(
                    mech.position, floor, TilePos::new(0, 0)
                );
    ...
    fn draw_arrow(&self, center_x: f32, center_y: f32, size: f32, angle_degrees: f32, color: Color) {
    ...
    pub fn render_debug_panel(&self, 
                            game_state: &GameState,
        ...
        if let Some(location) = current_player_location {
            match location {
                shared::types::PlayerLocation::OutsideWorld(pos) => {
                    draw_text(&format!("Location: Outside World"), panel_x + 10.0, y, 14.0, YELLOW);
                    y += 18.0;
                    draw_text(&format!("World: ({:.1}, {:.1})", pos.x / TILE_SIZE, pos.y / TILE_SIZE), panel_x + 10.0, y, 12.0, WHITE);
                    y += 18.0;
                }
                shared::types::PlayerLocation::InsideMech { mech_id, floor, pos } => {
                    draw_text(&format!("Location: Inside Mech"), panel_x + 10.0, y, 14.0, GREEN);
                    y += 18.0;
                    draw_text(&format!("Mech ID: {:.8}", mech_id.to_string()), panel_x + 10.0, y, 12.0, WHITE);
                    y += 18.0;
                    draw_text(&format!("Floor: {}", floor), panel_x + 10.0, y, 12.0, WHITE);
                    y += 18.0;
                    draw_text(&format!("Interior: ({:.1}, {:.1})", pos.x / TILE_SIZE, pos.y / TILE_SIZE), panel_x + 10.0, y, 12.0, WHITE);
                    y += 18.0;
                    ...
                    if let Some(mech) = game_state.mechs.get(mech_id) {
                        ...
                        let world_pos = world_tile.to_world();
                        draw_text(&format!("World Equiv: ({:.1}, {:.1})", world_pos.x / TILE_SIZE, world_pos.y / TILE_SIZE), panel_x + 10.0, y, 12.0, ORANGE);
                        y += 18.0;
                    }
                }
            }
        ...
        y += 18.0;
        draw_text("F3 - Start/Stop movement test", panel_x + 10.0, y, 12.0, WHITE);
        y += 18.0;
