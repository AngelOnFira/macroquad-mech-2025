...
fn render_mechs(game_state: &GameState, cam_x: f32, cam_y: f32, vision_system: Option<&ClientVisionSystem>) {
    for mech in game_state.mechs.values() {
        ...
        if let Some(vision) = vision_system {
            ...
            for (floor, interior_pos, visibility) in interior_tiles {
                if visibility > 0.1 {
                    render_visible_interior_tile(mech, floor, interior_pos, visibility, cam_x, cam_y);
                }
            }
        }
    }
...
fn render_visible_interior_tile(mech: &MechState, floor: u8, interior_pos: TilePos, visibility: f32, cam_x: f32, cam_y: f32) {
...
fn render_world_tiles(game_state: &GameState, cam_x: f32, cam_y: f32, vision_system: Option<&ClientVisionSystem>) {
    ...
    for (tile_pos, tile_visual) in &game_state.visible_tiles {
        ...
        if let Some(vision) = vision_system {
            let visibility = vision.get_visibility(*tile_pos);
            if visibility > 0.05 { // Only render if somewhat visible
...
fn render_resources(game_state: &GameState, cam_x: f32, cam_y: f32, vision_system: Option<&ClientVisionSystem>) {
...
fn render_projectiles(game_state: &GameState, cam_x: f32, cam_y: f32, vision_system: Option<&ClientVisionSystem>) {
...
fn render_players_in_world(game_state: &GameState, cam_x: f32, cam_y: f32, vision_system: Option<&ClientVisionSystem>) {
        ...
        if let PlayerLocation::OutsideWorld(pos) = player.location {
            ...
            if let Some(resource_type) = player.carrying_resource {
                ...
                if let Some(vision) = vision_system {
                    let tile_pos = pos.to_tile();
                    let visibility = vision.get_visibility(tile_pos);
                    resource_color = FogOfWarRenderer::apply_fog_to_color(resource_color, visibility);
                }
                draw_circle(
        ...
        for tx in start_x..end_x {
            ...
            if !vision_system.is_visible(tile_pos) {
                ...
                if edge_fade > 0.0 {
                    ...
                    draw_rectangle(tile_x, tile_y, TILE_SIZE, TILE_SIZE, FogOfWarRenderer::get_fog_overlay_color());
                }
            }
        }
    }
