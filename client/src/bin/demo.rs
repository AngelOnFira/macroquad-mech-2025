...
impl DemoMode {
        ...
        for x in -20..40 {
            for y in -20..40 {
                let tile = if rand::gen_range(0, 100) < 5 {
                    // Occasional rock
                    Tile {
                        visual: TileVisual::Wall { material: Material::Damaged },
                        blocks_vision: true,
                        walkable: false,
                    }
                } else {
                    // Grass
                    Tile {
                        visual: TileVisual::Floor { material: Material::Damaged, wear: 50 },
                        blocks_vision: false,
                        walkable: true,
                    }
                };
            ...
            for dy in 0..10 {
                ...
                let tile = Tile {
                    visual: TileVisual::Floor {
                        material: if is_entrance { Material::Reinforced } else { Material::Metal },
        ...
        for x in 0..10 {
            for y in 0..10 {
                ...
                let tile = if is_entrance {
                    // Entrance zone
                    Tile {
                        visual: TileVisual::Floor { material: Material::Reinforced, wear: 200 },
                        blocks_vision: false,
                        walkable: true,
                    }
                    ...
                    // Stairs up
                    Tile {
                        visual: TileVisual::Floor { material: Material::Reinforced, wear: 150 },
                        blocks_vision: false,
                        walkable: true,
                    }
                } else if is_wall {
                    ...
                    if is_window {
                        ...
                        Tile {
                            visual: TileVisual::Window { broken: false, facing },
                        ...
                        Tile {
                            visual: TileVisual::Wall {
                                material: if floor > 0 { Material::Reinforced } else { Material::Metal }
                            },
                            blocks_vision: true,
                            walkable: false,
                        }
                    ...
                    if let Some(station_type) = station {
                        Tile {
                            visual: TileVisual::Station { station_type, active: rand::gen_range(0, 2) == 0 },
                            blocks_vision: false,
                            walkable: true,
                        }
                        ...
                        // Turret
                        Tile {
                            visual: TileVisual::Turret { facing: Direction::Up, firing: false },
                            blocks_vision: true,
                            walkable: false,
                        }
                    } else {
                        Tile {
                            visual: TileVisual::Floor { material: Material::Metal, wear: 0 },
                            blocks_vision: false,
                            walkable: true,
                        }
                    }
    ...
    fn get_player_tile_pos(&self) -> (i32, i32) {
        ((self.player_x / TILE_SIZE).floor() as i32, (self.player_y / TILE_SIZE).floor() as i32)
    }
                    ...
                    for x in 0..10 {
                        ...
                        if has_los {
                            ...
                            if let Some(tile) = self.mech_tiles.get(&(floor, x, y)) {
                                if let TileVisual::Window { facing, broken: false } = tile.visual {
    ...
    fn is_walkable(&self, world_x: i32, world_y: i32) -> bool {
        match self.current_layer {
            LayerType::Ground => {
                self.ground_tiles.get(&(world_x, world_y))
                    .map(|t| t.walkable)
                    .unwrap_or(false)
            }
            LayerType::MechFloor(floor) => {
                ...
                if rel_x >= 0 && rel_x < 10 && rel_y >= 0 && rel_y < 10 {
                    self.mech_tiles.get(&(floor, rel_x, rel_y))
                        .map(|t| t.walkable)
                        .unwrap_or(false)
                } else {
                    false
    ...
    fn render_tile(&self, tile: &Tile, screen_x: f32, screen_y: f32, alpha: f32) {
        ...
        match &tile.visual {
            ...
            _ => {
                ...
                self.render_tile_visual_with_alpha(&tile.visual, screen_x, screen_y, TILE_SIZE, alpha);
            }
        }
    }
    
    fn render_tile_visual_with_alpha(&self, visual: &TileVisual, x: f32, y: f32, size: f32, alpha: f32) {
    ...
    async fn run(&mut self) {
        loop {
            ...
            let vel_magnitude = (self.player_velocity.0 * self.player_velocity.0 + 
                                self.player_velocity.1 * self.player_velocity.1).sqrt();
            ...
            let layers_to_render = if let Some(from_layer) = self.transition_from {
                vec![(from_layer, 1.0 - self.transition_progress), 
            ...
            for (layer, alpha) in layers_to_render {
                match layer {
                    ...
                    LayerType::MechFloor(floor) => {
                        ...
                        if alpha > 0.5 { // Only when this is the primary layer
                            for (pos, tile) in &self.ground_tiles {
                                if self.visible_tiles.contains(pos) {
                                    let world_x = pos.0 as f32 * TILE_SIZE;
                                    let world_y = pos.1 as f32 * TILE_SIZE;
            ...
            draw_text("Walk onto entrance/stairs to transition", 10.0, 75.0, 18.0, LIGHTGRAY);
            draw_text("Q/E: Go up/down when on stairs", 10.0, 95.0, 18.0, LIGHTGRAY);
