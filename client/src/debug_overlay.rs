...
impl DebugOverlay {
    ...
    fn render_server_state_panel(&mut self, ui: &mut Ui, game_state: &GameState) {
        ...
        ui.indent("server_state_indent", |ui| {
            ...
            let location_text = match game_state.player_location {
                PlayerLocation::OutsideWorld(pos) => format!("Outside at ({:.1}, {:.1})", pos.x, pos.y),
                PlayerLocation::InsideMech { floor, pos, .. } => {
                    format!("Inside Mech - Floor {} at ({:.1}, {:.1})", floor + 1, pos.x, pos.y)
                }
            };
            ...
            ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    ...
                    ui.add(TextEdit::multiline(&mut ascii_view.as_str())
    ...
    fn tile_to_ascii(&self, visual: &TileVisual) -> char {
        match visual {
            ...
            TileVisual::Station { station_type, active } => {
                match station_type {
                    StationType::Pilot => if *active { 'P' } else { 'p' },
                    StationType::WeaponLaser => if *active { 'L' } else { 'l' },
                    StationType::WeaponProjectile => if *active { 'T' } else { 't' },
                    StationType::Shield => if *active { 'S' } else { 's' },
                    StationType::Engine => if *active { 'E' } else { 'e' },
                    StationType::Repair => if *active { 'H' } else { 'h' },
                    StationType::Upgrade => if *active { 'U' } else { 'u' },
                    StationType::Electrical => if *active { 'C' } else { 'c' }, // 'C' for Circuit
                }
            },
            TileVisual::Turret { firing, .. } => if *firing { 'X' } else { 'x' },
    ...
    pub fn log_server_message(&mut self, message: &str) {
        ...
        self.message_history.push_back(format!("[{}] {}", self.message_counter, message));
