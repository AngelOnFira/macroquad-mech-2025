...
function init_plugins(plugins, wasm_memory, wasm_exports) {
    if (plugins == undefined)
        return;
    ...
    for (var i = 0; i < plugins.length; i++) {
        ...
        if (plugins[i].set_wasm_refs != undefined && plugins[i].set_wasm_refs != null) {
            plugins[i].set_wasm_refs(wasm_exports, wasm_memory);
        }
        ...
        WebAssembly.compileStreaming(req)
            ...
            .then(
                obj => {
                    ...
                    init_plugins(plugins, wasm_memory, wasm_exports);
                    obj.exports.main();
                })
            .catch(err => {
                console.error(err);
